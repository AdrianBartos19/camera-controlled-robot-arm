...
> **Status: work in progress / not fully working.** Camera-based object detection and coordinate tracking work correctly, but the robot arm's physical movement (inverse kinematics execution / motor control) does not work reliably. I no longer have access to the physical robot, so I'm unable to continue debugging or fixing the movement issues. Uploading this project as-is for reference.
